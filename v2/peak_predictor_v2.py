# peak_predictor.py
"""
Peak prediction - predicts peak electrical usage based on temperature weighting
"""
...
class PeakPredictorV2:
    """Predicts peak electrical usage with temperature weighting"""
    ...
    def find_max(self, df, at_time):
        """Find maximum peak and calculate predicted peak strength with temp weighting"""
        if df.is_empty():
            return df
        ...
        # Calculate max peak - only use actual load and forecast, NOT init_peak_mw
        ...
        # Apply temperature weighting to peak strength
        df = df.with_columns([
            (pl.col('predicted_peak_strength') * pl.col('temp_weight')).alias('predicted_peak_strength_weighted')
        ])

        df = df.with_columns([
            pl.when(pl.col('predicted_peak_strength_weighted') > 0)
            .then(pl.col('predicted_peak_strength_weighted'))
            .otherwise(0.0)
